...
count = 0
images = []  #Resim matrislerinin tutulacağı dizi.
classNo = [] #resim sınıflarının tutulacağı dizi
myList = os.listdir(path)  # Klasör içerisinde bulunan her bir sınıf klasörünün dosya yolu.
...
def myModel():
    ...
    model.add((Conv2D(no_Of_Filters, size_of_Filter, input_shape=(imageDimesions[0], imageDimesions[1], 1),
                      activation='relu')))
    # DAHA FAZLA DÖNÜŞÜM KATMANLARI EKLEMEK = DAHA AZ ÖZELLİKLER ANCAK DOĞRULUĞUN ARTMASINA NEDEN OLABİLİR
